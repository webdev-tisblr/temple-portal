...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 bg-temple">
    <x-breadcrumb
        :items="[
            ['label' => __('events.title'), 'url' => route('events.index')],
            ['label' => $event->title],
        ]"
        class="mb-6" />
    ...
    <div class="card-sacred p-6 sm:p-10">
        ...
        @if($event->media->isNotEmpty())
            <div class="mt-10 pt-8 border-t border-amber-900/20">
                <p class="text-sm font-semibold text-amber-100/50 mb-4">{{ __('events.gallery') }}</p>
                            ...
                            <div class="col-span-2 sm:col-span-3 relative aspect-video rounded-xl overflow-hidden bg-black border border-amber-900/20">
                                @if($isYt && $ytId)
                                    <iframe class="absolute inset-0 w-full h-full" src="https://www.youtube-nocookie.com/embed/{{ $ytId }}"
                                            title="{{ $event->title }}" frameborder="0" loading="lazy" allowfullscreen></iframe>
                                @else
        ...
        {{-- Share Section --}}
        <div class="mt-10 pt-8 border-t border-amber-900/20">
            <p class="text-sm font-semibold text-amber-100/50 mb-3">{{ __('events.share') }}</p>
            <div class="flex flex-wrap gap-3">
                ...
                <a href="https://api.whatsapp.com/send?text={{ urlencode($event->title . ' - ' . request()->url()) }}"
                   ...
                   class="inline-flex items-center gap-2 px-4 py-2 bg-green-500 text-white text-sm font-medium rounded-lg hover:bg-green-600 transition">
                    ...
                    {{ __('events.whatsapp_share') }}
                </a>

                {{-- Copy Link --}}
                <button onclick="navigator.clipboard.writeText('{{ request()->url() }}').then(()=>alert('{{ __('events.link_copied') }}'))"
