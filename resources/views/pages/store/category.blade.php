...
<x-page-header
    :breadcrumb="[
        ['label' => __('footer.temple_store'), 'url' => route('store.index')],
        ['label' => $category->name],
    ]"
    :title="$category->name"
    ...
    <div class="mb-8" x-data="{
        ...
        <form method="GET" action="{{ route('store.category', $category->slug) }}"
            ...
            <select name="sort" x-model="sort"
                ...
                <option value="newest" class="bg-stone-900 text-amber-100">{{ __('store.newest') }}</option>
                <option value="price_asc" class="bg-stone-900 text-amber-100">{{ __('store.price_asc') }}</option>
                <option value="price_desc" class="bg-stone-900 text-amber-100">{{ __('store.price_desc') }}</option>
            </select>
            ...
            <div class="flex flex-wrap gap-2 mt-3">
                ...
                @if(request('sort', 'newest') !== 'newest')
                    <a href="{{ route('store.category', ['slug' => $category->slug, 'search' => request('search')]) }}"
                       class="inline-flex items-center gap-1 px-3 py-1 text-xs rounded-full bg-amber-900/30 text-amber-400 border border-amber-800/30 hover:border-amber-600 transition">
                        @if(request('sort') === 'price_asc') {{ __('store.price_asc') }} @else {{ __('store.price_desc') }} @endif
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                    </a>
                @endif
            </div>
