            ...
            <div class="lg:sticky lg:top-24 card-sacred p-6">
                ...
                <div class="border-t border-amber-900/20 pt-6">
                    @auth('devotee')
                        ...
                        @endif
                    @else
                        <a href="{{ login_url() }}" class="flex items-center justify-center w-full px-8 py-3 btn-divine gap-2">
...
function productPage() {
    const primaryImage = @json($product->image_path ? image_url($product->image_path) : null);
    const galleryImages = @json($product->images->sortBy('sort_order')->pluck('image_path')->map(fn($p) => image_url($p))->values());
