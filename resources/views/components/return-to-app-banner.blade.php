{{--
    ...
    The "Open app" button fires the patadiyahanumanji:// scheme and is
    gated behind the app_scheme_enabled System Setting — keep it OFF
    until the scheme-registered app build is rolled out, or pre-scheme
    installs get Safari's "invalid address" alert.
--}}
@php
...
>
    <div class="mx-auto max-w-md flex items-center gap-3 rounded-2xl bg-[#7A1E1E] text-[#FBF5EA] shadow-2xl px-4 py-3">
        ...
        @if($schemeEnabled)
            <button @click="openApp()" class="shrink-0 rounded-full bg-[#FBF5EA] text-[#7A1E1E] text-sm font-bold px-4 py-2">
                {{ __('common.return_to_app_button') }}
            </button>
        @endif
        <button @click="dismiss()" aria-label="Close" class="shrink-0 text-amber-100/60 hover:text-amber-100 p-1">
