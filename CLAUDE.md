...
- **Queue is Redis with 2 always-on Supervisor workers** (VPS since 2026-07-25; `--queue=otp,default,broadcast`, OTP has priority). `config/queue.php` reads `QUEUE_CONNECTION` — **prod must set `QUEUE_CONNECTION=redis`**. It was hardcoded to `sync` for Hostinger until 2026-08-07, which meant the workers sat idle and every job ran inline; if the workers ever look busy-but-idle again, check that value first. Setting it back to `sync` is the rollback. Do NOT add a scheduled `queue:work` — the Supervisor workers own the queue.
- **Every `withoutOverlapping()` in `routes/console.php` passes an explicit expiry.** Laravel's default is 24h, and `seva:dispatch-reminders` permanently skips anything more than 12h late (`--max-late-minutes=720`) — so a leaked lock from a SIGKILLed run would silently bin half a day of reminders. Keep the expiry well under 12h on any new scheduled task. Notification dispatches go through a **transactional outbox** (`temple_notification_outbox` + `notifications:relay-outbox`) when `NOTIFICATIONS_VIA_QUEUE=true` (prod). Contexts carrying `_attachments` stay inline by design. Deploys run `php artisan queue:restart` so workers pick up new code. Guest pages are cached twice: `CacheGuestResponse` middleware (per-locale, 120s) + a Cloudflare edge Cache Rule (cookie-aware). Do not add forms/CSRF surfaces to the cached-path whitelist without removing those paths from both layers.
- **Scheduled launch**: `launch_at` (SystemSetting, IST wall clock, set on the dashboard widget) shows a countdown on the coming-soon page and opens the site by itself. The **middleware** is what opens it — `ComingSoonMode` lets traffic through the instant `launch_at` passes, whatever the flag says — so a wedged scheduler cannot delay a launch. `site:launch` (every minute) only flips the stored flag afterwards and purges Cloudflare; set `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ZONE_ID` in prod `.env` or that purge is skipped and the edge keeps serving the 503 page.
- **Flipping `coming_soon_mode` REQUIRES a Cloudflare purge** (dashboard → Caching → Configuration → Purge Everything). `ComingSoonMode` answers **503**, and the edge Cache Rule covers exactly `/`, `darshan`, `gallery`, `projects`, `trustees`, `pages/*` — so Cloudflare's stale-on-error keeps serving its cached copies of those six shapes indefinitely and the site looks half-hidden. Same mechanism as the 2026-07-28 outage in operator step 3. Turn **Always Online OFF** during the window too, or archived copies come back after the purge. Applies in both directions (on *and* off at launch).
- **`resources/app-l10n/*.json` is GENERATED, never hand-edited.** It is a copy of the Flutter app's `assets/l10n/{gu,hi,en}.json` and is the only thing the admin "App Text Fixes" screen knows about the app's wording — it builds the key picker and the "what the app shows today" panel from it. A stale copy fails silently: strings shipped since the copy was taken cannot be selected at all, and changed strings display the old text (which reads to an admin as "the panel isn't saving my updates"). Regenerate it on every store build with `php artisan app:sync-l10n-snapshot`; `php artisan app:sync-l10n-snapshot --check` exits 1 on drift and is asserted by `tests/Feature/AppStringOverrideTest`.
...
- Deploy takes a pre-migration DB snapshot (`backup:run --only-db`) BEFORE `migrate --force`, then runs the reminder backfills + role/permission sync, then clears/rebuilds caches.
- **No CONTENT seeder runs on deploy** (2026-08-13). Notification templates, gallery categories and seva categories are fresh-install setup only (`DatabaseSeeder`). They were all `firstOrCreate` so they never overwrote an admin edit, but they resurrected rows the admin had deliberately deleted, every deploy. Production content is owned by the admin panel — do not add a content seeder back to `deploy.yml`. The only things that still write rows are: `migrate` (once each), the two reminder backfills (real bookings), `seva:restamp-fullday-reminders` (an UPDATE), `RolePermissionSeeder` and `shield:generate` (permission NAMES only, never content).
